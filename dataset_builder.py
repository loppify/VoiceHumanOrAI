...
class ServerManagedProvider(TTSProvider):
    ...
    async def _ensure_venv(self):
        ...
        if is_system_python and not os.path.exists(venv_path):
            ...
            subprocess.run([python_exe, "-m", "pip", "install", "fastapi", "uvicorn", "pydantic", "requests"], check=True)
        ...
        try:
            ...
            subprocess.run([python_exe, "-c", "import transformers; assert int(transformers.__version__.split('.')[0]) < 5"], check=True, capture_output=True)
        except:
            ...
            if os.path.exists(req_file):
                ...
                if res.returncode != 0:
                    ...
                    subprocess.run([python_exe, "-m", "pip", "install", "torch", "torchaudio", "transformers<5.0", "sentencepiece", "soundfile", "onnxruntime"], check=True)
            else:
                subprocess.run([python_exe, "-m", "pip", "install", "torch", "torchaudio", "transformers<5.0"], check=True)
                ...
                if not await self._is_running():
                    ...
                    try:
                        ...
                        if attempt < max_retries - 1:
                            logger.warning(f"Сервер не відповів. Можливо, пошкоджено venv. Спроба перевстановлення залежностей...")
...
async def build_dataset(samples: int, provider_name: str, dataset_name: str, config_name: str, split_name: str, human_dir: str = "dataset/human", ai_dir: str = "dataset/ai"):
    ...
    ds = load_dataset(dataset_name, config_name, split=split_name, streaming=True, token=HF_TOKEN).shuffle(buffer_size=1000)
            ...
            async with provider_class() as provider:
                for row in ds:
                    ...
                    try:
                        ...
                        if config_name.lower().startswith("en") and row_lang and not row_lang.startswith("0"): # 0 is en-US in minds14
                        ...
                        try:
                            ...
                            if he.response.status_code == 400:
                                logger.warning(f"Провайдер відхилив текст (можливо, непідтримувана мова): {text[:30]}...")
